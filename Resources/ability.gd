...
func ability_init(in_battle_event: BattleEvent, in_trigger: Trigger) -> void:
    self.battle_event = in_battle_event
    self.trigger = in_trigger
    self.when_to_trigger = BattleResources.WhenToTrigger.values()[ randi() % BattleResources.WhenToTrigger.size() ]
...
func apply_ability(current_slot: BeingSlot, other_slot: BeingSlot) -> void:
        ...
        await other_slot.being_stats.ability_tween(.75)
