...
func apply_abilitie_grid(current_slot: BeingSlot, grid: BeingGrid, when_to_trigger: BattleResources.WhenToTrigger) -> void:
    for grid_slot: BeingSlot in grid.alive_beings:
        for current_ability: Ability in all_abilities:
            if !current_slot.compare_slots(grid_slot) and current_ability.is_triggered(current_slot, grid_slot, when_to_trigger):
